...
def renderGame(screen, board, boardRows, boardCols, gameOver, winner):
    drawFigures(screen, board, boardRows, squareSize, boardCols,
                White, crossWidth, circleRadius, circleWidth)
    drawLines(screen, squareSize, boardRows, White if not gameOver else (
        Green if winner == 1 else Red if winner == 2 else Blue), WIDTH, HEIGHT, LINEWIDTH)
